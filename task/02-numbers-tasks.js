...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    let car=Math.sqrt(x1*x1+y1*y1);
    let cas=Math.sqrt(x2*x2+y2*y2);
    let cosin=(x1*x2+y2*y1)/(car*cas);
    let arr=Math.acos(cosin);
    return arr;
    throw new Error('Not implemented');
}
...
for(let i=b.length-1;i>b.length-1-pow;i--)
{
b.splice(i, 1, 0);
}
